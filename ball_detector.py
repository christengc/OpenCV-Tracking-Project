...
def _preprocess_patch_for_knn(patch, patch_size=20):
    """Convert patch to flattened normalized features for KNN prediction."""
    if patch is None:
        return None
    if patch.ndim == 3:
...
def _classify_contour_patch_knn(inputImage, cx, cy, knn_model, patch_size=20):
    ...
    if knn_model is None:
        if not hasattr(_classify_contour_patch_knn, "_warned_no_model"):
            print("[KNN] knn_model is None during contour classification")
            _classify_contour_patch_knn._warned_no_model = True
        return False
    half = patch_size // 2
    ...
    features = _preprocess_patch_for_knn(patch, patch_size=patch_size)
    if features is None:
        return False
    try:
        ...
        if not hasattr(_classify_contour_patch_knn, "_warned_predict_error"):
            ...
            _classify_contour_patch_knn._warned_predict_error = True
        return False
...
def identifyContours(inputImage, binaryMask, output, tracker, last_ball, debug):
    ...
    for cnt in contours:
        ...
        if score > best_score:
            ...
            best_ball = (cx, cy, r)
        # Passed all checks
        contour_statuses.append((cnt, 'knn_ball' if knn_is_ball else 'passed'))
